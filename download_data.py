...
#: floor (the fraction then scales the whole estimate).
DATASETS = {
...
def select_shards(files, fraction: float, repo_id: str = "the repo") -> dict:
    ...
    shards, whatever the fraction. A train-only repo gives an empty
...
def count_distinct_labels(split):
    ...
    column. ``Dataset.unique`` reads only that column."""
...
def main(argv=None) -> int:
    ...
    if fraction >= 1.0:
        ...
        for split, names in selected.items():
            if not names:
                continue                                   # no validation split in this repo
