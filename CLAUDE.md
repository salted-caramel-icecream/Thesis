...
- `PVT_Tutelmoe_v12_standalone.ipynb` is GENERATED from `pvt_moe/` by `tools/make_v12_notebook.py` — self-contained (no package import), every code cell a package file inlined verbatim, with "Δ since v10" markdown cells striking through the old lines. Never hand-edit its code cells: edit the package and regenerate. `tests/verify_v12_notebook.py` (not test_*) executes its cells and asserts bit-for-bit model parity with the package, run-name parity, and that it learns.
