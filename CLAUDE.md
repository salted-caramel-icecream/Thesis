...
- `notebooks/` — `v11_train.ipynb` is the supervised launcher (from a checkout), `colab_train.ipynb` the same from a pinned `pip install git+...` for a machine with no clone, `03_ssl_pretrain.ipynb` the SSL one (edit only their CONFIG cells, via json load/dump); `archive/` — v9/v10 notebooks and the process-history documents, unmaintained
